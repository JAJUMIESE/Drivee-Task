/* function findSuitableOrders(driverRoute, orders) {
  ...
  for (let order of orders) {
    ...
    if (driverAndOrderDistance < shortestDistance) {
      ...
      closestOrder = order; // изменить на добавлять элементы в массив 
    }
  }
...
let orders = [
  { A: { x: 2, y: 2 }, B: { x: 5, y: 5 }  },
  { A: { x: 8, y: 8 }, B: { x: 12, y: 12 } },
  { A: { x: 3, y: 3 }, B: { x: 7, y: 7 } }
];

let suitableOrders = findSuitableOrders(driverRoute, orders); // массив подходящих заказов 
console.log(suitableOrders); // вывести по убыванию кратчайший и максимальый */
